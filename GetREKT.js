  ...
  function component(object) {
    ...
    this.appendEpisode = function (element, episodes, choice, items, params, is_append) {
      ...
      if (episode) {
        ...
        element.descr = (episode.overview || '');
      ...
      if (serial && !episode && filter_find.season.length) {
        ...
        // Netflix-like: always try to show a preview image
    ...
    */
    ...
    function add() {
      ...
      if (!window.__getrekt_svg_clamp) {
        window.__getrekt_svg_clamp = true;

        var clampGetREKTSvgs = function clampGetREKTSvgs(root) {
          try {
            var scope = root && root.querySelectorAll ? root : document.body;
            if (!scope) return;

            var svgs = scope.querySelectorAll('.online_getrekt svg, .onlines_v1 svg, .torrent-item__viewed svg');
            for (var i = 0; i < svgs.length; i++) {
              var svg = svgs[i];
              if (!svg || !svg.getBoundingClientRect) continue;

              // Allow larger folder icon
              if (svg.closest && svg.closest('.online_getrekt__folder')) {
        ...
        try {
          var svgClampObserver = new MutationObserver(function (mutations) {
            for (var m = 0; m < mutations.length; m++) {
              var mut = mutations[m];
              if (!mut.addedNodes || !mut.addedNodes.length) continue;
              for (var n = 0; n < mut.addedNodes.length; n++) {
                var node = mut.addedNodes[n];
                if (node && node.nodeType === 1) clampGetREKTSvgs(node);
              }
            }
          });
          svgClampObserver.observe(document.body, { childList: true, subtree: true });
        } catch (e) {}
      }
